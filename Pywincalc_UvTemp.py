...
print(len(inside_temp_l),len(outside_temp_l),len(u_value_l2))

contour = plt.contourf(outside_temp_l,inside_temp_l,u_value_l2)
cbar = plt.colorbar(contour)
cbar.set_label('U-Value')
plt.xlabel('Indoor Temperature [K]')
plt.ylabel('Outdoor Temperature [K]')
plt.title('U-Value as a Function of Indoor and Outdoor Temperture')
plt.show()
